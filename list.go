...
func Read(r io.ReadSeeker) ([]os.FileInfo, error) {
	var header header
	...
	if _, err := r.Seek(int64(header.IndexOffset), 0); err != nil {
		return nil, err
	}

	var numBlockTablePointers uint32

	if err := binary.Read(r, binary.BigEndian, &numBlockTablePointers); err != nil {
		return nil, err
	}
	...
	if _, err := r.Seek(int64(header.VarsOffset), 0); err != nil {
		return nil, err
	}
	...
	for i := 0; i < int(numVars); i++ {
		...
		if string(name[0:]) == "Paths" {
			var tree tree

			if err := blockPointers.lookup(r, int(index), &tree); err != nil {
				return nil, err
			}

			var paths paths

			if err := blockPointers.lookup(r, int(tree.Child), &paths); err != nil {
				return nil, err
			}
			...
			for paths.IsLeaf == 0 {
				if err := blockPointers.lookup(r, int(indices[0].Index0), &paths); err != nil {
					return nil, err
				}
			...
			for {
				for j := 0; j < int(paths.Count); j++ {
					...
					if err := blockPointers.lookup(r, int(index1), &parent); err != nil {
						return nil, err
					}
					...
					if err := blockPointers.lookup(r, int(index0), &pathInfo1); err != nil {
						return nil, err
					}

					var pathInfo2 pathInfo2

					if err := blockPointers.lookup(r, int(pathInfo1.Index), &pathInfo2); err != nil {
						return nil, err
					}
				...
				if paths.Forward == 0 {
					...
					err := blockPointers.lookup(r, int(paths.Forward), &paths)
